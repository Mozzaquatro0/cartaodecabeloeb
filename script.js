...
const urlBrasao = "https://upload.wikimedia.org/wikipedia/commons/thumb/9/93/Bras%C3%A3o_do_Ex%C3%A9rcito_Brasileiro.svg/200px-Bras%C3%A3o_do_Ex%C3%A9rcito_Brasileiro.svg.png";
...
/* Funções Core de Organização e Sincronização de Banco de Dados */
...
function removerMilitar(index) {
    if (confirm("Remover este militar da lista?")) {
        efetivoSU.splice(index, 1); salvarBancoDados(); renderizarSistema();
    }
}

/* Operações de Backup Compartilhado (JSON e TXT) */
        ...
        try {
            ...
            if (dadosImportados.subunidade && Array.isArray(dadosImportados.efetivo)) {
                if (confirm("Deseja substituir a lista atual pelo backup?")) {
                    ...
                    ordenarEfetivo(); salvarDadosGerais(); salvarBancoDados(); renderizarSistema();
...
function importarArquivoTexto(input) {
    ...
    reader.onload = function (e) {
        ...
        let adicionados = 0; let mantidos = 0; let excluidos = 0;
        ...
        nomesNoTxt.forEach(mTxt => {
            ...
            if (!jaExiste) {
                efetivoFiltrado.push({ grad: mTxt.grad, nome: mTxt.nome, dataCartao: "", imprimir: false, qtdCartoes: 0, historicoOcorrencias: [] });
                adicionados++;
            } else { mantidos++; }
        });
        efetivoSU = efetivoFiltrado; ordenarEfetivo(); salvarBancoDados(); renderizarSistema();
        input.value = "";
...
function resetarSistemaCompleto() {
    if (confirm("⚠ ATENÇÃO: Deseja apagar permanentemente TODO o efetivo, históricos e registros?")) {
        if (confirm("CONFIRMAÇÃO FINAL: Limpar banco de dados para o próximo ano instrucional?")) {
            efetivoSU = []; salvarBancoDados(); renderizarSistema(); alert("Sistema zerado.");
        }
    }
}

/* Fluxo de Inclusão Manual Dinâmica */
...
/* Métricas de Datas, Validade e Indicadores */
function calcularDataVencimento(dataString) {
    if (!dataString) return "";
    const dataCartao = new Date(dataString);
    dataCartao.setMonth(dataCartao.getMonth() + 12);
    return dataCartao.toISOString().split('T')[0].split('-').reverse().join('/');
}

function verificarValidade(dataString) {
    if (!dataString) return "SEM CARTÃO";
    const dataCartao = new Date(dataString); const hoje = new Date();
    dataCartao.setMonth(dataCartao.getMonth() + 12);
    return hoje > dataCartao ? "VENCIDO" : "VÁLIDO";
}

function atualizarIndicadores() {
    let total = efetivoSU.length; let validos = 0; let vencidos = 0; let semCartao = 0;
    efetivoSU.forEach(m => {
        let status = verificarValidade(m.dataCartao);
        if (status === 'VÁLIDO') validos++; else if (status === 'VENCIDO') vencidos++; else semCartao++;
    });
...
/* Gerenciamento de Históricos e Ocorrências */
...
function salvarOcorrenciaModal() {
    ...
    if (!texto) { alert("Por favor, digite o motivo ou o relato antes de salvar!"); return; }
    ...
    efetivoSU[militarSelecionadoIndex].historicoOcorrencias.unshift({ data: dataFormatada, texto: texto.toUpperCase() });
...
function editarOcorrenciaDados(ocIndex) {
    ...
    if (!novoTexto) {
        alert("O texto não pode ficar vazio. Se deseja remover, utilize o botão de lixeira (🗑️).");
        return;
    }

    if (novoTexto !== textoBase) {
        const hoje = new Date();
        const dataAlteracao = hoje.toLocaleDateString('pt-BR') + ' às ' + hoje.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
...
/* Motores de Filtragem e Mudança de Aba */
...
function gerarTabelaHTML(meses) {
    let h = `<table>
        <tr>
            ...
            <th style="width: 13%;">DIA</th>
            <th style="width: 30%;">Rubrica do Cmt SU</th>
            <th style="width: 13%;">DIA</th>
            <th style="width: 30%;">Rubrica do Cmt SU</th>
        </tr>`;
    meses.forEach(m => {
        h += `<tr><td rowspan="3" class="mes-celula">${m}</td><td></td><td></td><td></td><td></td></tr>
...
function imprimirCartoesAvulsoDadosPDF() {
    ...
    const css = `
        *{margin:0;padding:0;box-sizing:border-box}@page{size:148mm 105mm;margin:0}
        ...
        .grad-field,.num-field,.nome-field{font-family:'Times New Roman',Times,serif;font-size:10.5pt;text-align:center;border:none;background:transparent}
...
function imprimir4VersoBranco() {
    ...
    const gerarTabela = (meses) => {
        let h = `<table><thead><tr><th style="width:14%">MÊS</th><th style="width:13%">DIA</th><th style="width:30%">Rubrica do Cmt SU</th><th style="width:13%">DIA</th><th style="width:30%">Rubrica do Cmt SU</th></tr></thead><tbody>`;
        meses.forEach(m => {
            h += `<td><td rowspan="3" class="mes-celula">${m}</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr><tr><td style="height:28px">&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr><tr><td style="height:28px">&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>`;
        });
        return h + `</tbody></table>`;
    };

    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>4x Verso Branco</title><style>
        *{margin:0;padding:0;box-sizing:border-box}@page{size:A4 landscape;margin:0}
        body{margin:0;background:white}.print-page{width:297mm;height:210mm;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:4mm;padding:8mm}
        .card-container{display:grid;grid-template-columns:1fr 1fr;border:1px solid #000;height:94mm}
        .card-half{padding:3mm;display:flex;flex-direction:column;position:relative}
        .verso-watermark{position:absolute;top:0;left:0;right:0;bottom:0;background-image:url('${urlBrasao}');background-repeat:no-repeat;background-position:center;background-size:55mm 55mm;opacity:0.05;pointer-events:none;z-index:1}
        .verso-half{padding:3mm 4mm;justify-content:center;position:relative}
        .verso-half table{width:100%;height:100%;border-collapse:collapse;font-size:7pt;text-align:center;background:transparent;position:relative;z-index:2}
        .verso-half table,.verso-half th,.verso-half td{border:1px solid black}
        .verso-half th{font-weight:bold;height:8%;text-transform:uppercase;background:rgba(240,240,240,0.6)}
        .verso-half td{height:5.1%}.mes-celula{font-weight:bold;font-size:7.5pt;background-color:rgba(250,250,250,0.6)}
        .table-side-left{border-right:1px dashed #bbb}
    </style></head><body><div class="print-page">`;

    let cards = '';
    for (let i = 0; i < 4; i++) {
        cards += `<div class="card-container"><div class="card-half table-side-left verso-half"><div class="verso-watermark"></div>${gerarTabela(m1)}</div><div class="card-half verso-half"><div class="verso-watermark"></div>${gerarTabela(m2)}</div></div>`;
    }

    printWindow.document.write(html + cards + `</div></body></html>`);
    printWindow.document.close();
    printWindow.print();
}
...
function renderizarSistema() {
    ...
    if (abaAtual === 'ficha') {
        ...
        efetivoFiltradoParaExibicao.forEach((militar) => {
            ...
            let statusClass = 'status-sem-cartao', statusTexto = '🛇 SEM CARTÃO';
            if (status === 'VÁLIDO') { statusClass = 'status-valido'; statusTexto = '✓ VÁLIDO'; }
            else if (status === 'VENCIDO') { statusClass = 'status-vencido'; statusTexto = '⚠ VENCIDO'; }
            ...
            // Calcula a data de validade para o tooltip
            let tooltipTexto = "";
        ...
        // Atualiza o estado do checkbox "todos" após renderizar
        atualizarCheckboxTodos();
    }
    if (abaAtual === 'frente') {
        for (let p = 0; p < totalPaginas; p++) {
            let pageDiv = document.createElement('div'); pageDiv.className = 'print-page';
            for (let c = 0; c < 4; c++) {
                let idx = (p * 4) + c; let militar = listaParaImprimir[idx];
                let nomeExibir = militar ? militar.nome : ''; let gradExibir = militar ? militar.grad : '';
                ...
                let contentHTML = militar ? `
                        ...
                        <div class="card-half content-side frente-padding">
                            ...
                            <div class="inline-group">
                                <div class="form-group" style="width: 65%;"><label>Grad.:</label><input type="text" value="${gradExibir}"></div>
                                <div class="form-group" style="width: 32%;"><label>N°:</label><input type="text" value=""></div>
                            </div>
                            <div class="form-group"><label>NOME:</label><input type="text" value="${nomeExibir}"></div>
        ...
        // Calcula quantas páginas são necessárias (cada página tem 4 cartões)
        const totalPaginasPrint = Math.ceil(listaParaImprimir.length / 4);
        ...
        for (let p = 0; p < totalPaginasPrint; p++) {
            ...
            // Preenche os 4 slots da página
            for (let c = 0; c < 4; c++) {
                const idx = (p * 4) + c;
                const militar = listaParaImprimir[idx];

                if (militar) {
                    // Mostra o cartão do militar
                    const contentHTML = `
                ...
                `;
                    ...
                    // Não mostra nada (espaço vazio sem borda)
...
document.addEventListener('DOMContentLoaded', function () {
    if (efetivoSU.length === 0) {
        efetivoSU = [
            { grad: "CB", nome: "SILVA", numero: "", dataCartao: "", imprimir: false, qtdCartoes: 0, historicoOcorrencias: [] },
            { grad: "SD EP", nome: "SANTOS", numero: "", dataCartao: "", imprimir: false, qtdCartoes: 0, historicoOcorrencias: [] },
            { grad: "SD EV", nome: "OLIVEIRA", numero: "", dataCartao: "", imprimir: false, qtdCartoes: 0, historicoOcorrencias: [] }
        ];
        salvarBancoDados();
    }
    ordenarEfetivo();
...
// Marcar/Desmarcar todos os militares da página atual
function marcarDesmarcarTodosPagina(marcar) {
    ...
    if (modificados > 0) {
        salvarBancoDados();
        // Não precisa renderizar tudo, apenas atualiza o estado dos checkboxes
        // Mas para garantir consistência, renderizamos novamente
        renderizarSistema();
    }
}

// Atualiza o estado do checkbox "todos" baseado nos checkboxes individuais
function atualizarCheckboxTodos() {
    ...
    if (total === 0) {
        ...
        checkboxTodos.checked = false;
        checkboxTodos.indeterminate = true; // Estado "meio marcado"
    } else {
        checkboxTodos.checked = false;
        checkboxTodos.indeterminate = false;
    }
...
// Função toggleImprimir modificada para atualizar o checkbox "todos"
// Substitua a função toggleImprimir existente por esta:
function toggleImprimir(index, checkbox) { 
    efetivoSU[index].imprimir = checkbox.checked; 
    salvarBancoDados();
    atualizarCheckboxTodos(); // Atualiza o estado do checkbox "todos"
}